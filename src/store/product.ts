...
export const fetchProductsDeleteFromCart = createAsyncThunk<string, {id: string}>("products/fetchProductsDeleteFromCart", async (params) => {
    axios.delete(`https://676b9a97bc36a202bb85137e.mockapi.io/cart/${params.id}`)
    return params.id
})
export const fetchProductsAddToFavorites = createAsyncThunk<ICard[], {obj: ICard}>("products/fetchProductsAddToFavorite", async (params) => {
...
export const {selectError, selectLoading, selectProducts, selectProductsCart, selectProductsFavorites, selectProductsOrders} = productSlice.selectors